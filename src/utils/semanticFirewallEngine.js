...
const fuse = new Fuse(vocabKeys, { threshold: 0.4, includeScore: true })
...
export function scoreQuery(query, department, sessionMemory = {}) {
  ...
  for (const word of words) {
    // Clean word: keep Latin letters and Devanagari characters
    ...
    // Skip pure Devanagari words for glossary matching (they won't be in the English glossary)
    const isDevanagari = /^[\u0900-\u097F]+$/.test(cleanWord)
    if (isDevanagari) {
    ...
    if (vocab[cleanWord]) {
      ...
      if (fuzzyResults.length > 0 && fuzzyResults[0].score < 0.35) {
        ...
        // Only add unknown penalty for words that look like they could be enterprise terms
  ...
  for (const term of detectedTerms) {
    ...
    if (prior) {
      const currentMeanings = vocab[term]?.map(d => d.dept) || []
      if (prior.resolvedDept && currentMeanings.includes(prior.resolvedDept)) {
  ...
  // Department weighting - if dept selected and matches, reduce ambiguity
  if (department && detectedTerms.length > 0) {
    const deptMatch = detectedTerms.some(term =>
      vocab[term]?.some(def => def.dept === department)
    )
    if (deptMatch) {
  ...
  return {
    ...
    wordAnalysis,
    language: langResult,
    trigger: finalScore > 60 ? 'multiple glossary defs' : finalScore > 40 ? 'moderate ambiguity' : 'resolved',
    action: finalScore > 60 ? 'clarification required' : finalScore > 40 ? 'gemini with warning' : 'passed to gemini',
